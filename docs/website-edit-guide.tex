...
\section*{12. How to release a new version}
In the normal setup, yes: committing and pushing to \texttt{main} is what releases the site.

Typical workflow:
\begin{enumerate}
\item Edit content locally.
\item Preview locally with \texttt{hugo server -D}.
\item Build locally with \texttt{hugo --gc --minify}.
\item Commit your changes.
\item Push to \texttt{main}.
\item Wait for the GitHub Actions workflow to finish.
\item Open \texttt{https://bertilledaran.github.io/} and hard-refresh the page.
\end{enumerate}
...
If the workflow fails, open the repository's Actions tab and inspect the \texttt{Deploy Hugo site to Pages} run.

\section*{13. Common update recipes}
...
\section*{14. Notes on old demo content}
